...
public class ListAdapter extends ArrayAdapter<ListModel> {
    ...
    public View getView(final int position, View convertView, final ViewGroup parent) {
        ...
        holder.med.setText(model.getMed()+ " "+ model.getUnit());
       /* Double up = roundOff(Double.parseDouble(model.getPr()) / Double.parseDouble(model.getUnit()));*/
         Double up = roundOff(Double.parseDouble(model.getAmount()) / Double.parseDouble(model.getQty()));
        holder.pr.setText(String.valueOf(up));
                    ...
                    public void onClick(DialogInterface dialog, int id) {
                        ...
                        if (!qt.equals("") && !qt.equals("0")) {
                            if (Integer.parseInt(qt) <= Integer.parseInt(model.getOrgqty())) {
                               ...
                               /* Double u = Double.valueOf(model.getAmount());
                                double am = Double.valueOf(u) * Double.valueOf(p);
                                am = roundOff(am);
                                Trans.totamount = (Trans.totamount - Double.valueOf(a)) + am;
                                Trans.totamount = roundOff(Trans.totamount);*/


                                double am = Double.valueOf(qt) * Double.valueOf(up);
                                am = roundOff(am);
                                Trans.totamount = (Trans.totamount - Double.valueOf(a)) + am;
                                Trans.totamount = roundOff(Trans.totamount);
